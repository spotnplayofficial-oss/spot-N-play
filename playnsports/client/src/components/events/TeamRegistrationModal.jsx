...
const TeamRegistrationModal = ({ event, teamSize, price, onClose, onSubmit }) => {
  ...
  const [form, setForm] = useState({
    ...
    captainName: '',
    captainMobile: '',
    players: Array.from({ length: others }, () => ({ name: '', mobile: '' })),
  });
  ...
  const submit = async (e) => {
    ...
    try {
      await onSubmit({
        ...
        players: form.players.map((p) => ({ name: p.name.trim(), mobile: p.mobile.trim() })),
      });
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
      <form className="w-full max-w-xl rounded-3xl p-6 max-h-[90vh] overflow-y-auto" style={{ background: 'var(--glass-bg, #101010)', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 24px 80px rgba(0,0,0,0.55)' }} onClick={(e) => e.stopPropagation()} onSubmit={submit}>
        <h2 className="text-lg font-bold mb-1" style={{ fontFamily: "'Bebas Neue', cursive", letterSpacing: 1 }}>REGISTER YOUR TEAM — {event.title?.toUpperCase()}</h2>
        <p className="text-xs mb-4" style={{ color: '#9ca3af' }}>{teamSize} players per team · Entry ₹{price ?? event.price} per team · One ticket covers whole team</p>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="mt-1">
            <p className="text-[10px] uppercase tracking-widest mb-2" style={{ color: '#9ca3af' }}>Other Players — {others} required</p>
